...
def load_and_concatenate_data(filepath):
    with h5py.File(filepath, 'r') as hdf:
        ...
        if os.path.exists('loadsm.dat'):
            os.remove('loadsm.dat')
        if os.path.exists('solutionsm.dat'):
            os.remove('solutionsm.dat')

        # Create memmap arrays for loads and solutions
        loads = np.memmap('loadsm.dat', dtype=sample_load.dtype, mode='w+', shape=loads_shape)
        solutions = np.memmap('solutionsm.dat', dtype=sample_solution.dtype, mode='w+', shape=solutions_shape)
...
loads = torch.tensor(loads, dtype=dtype, device=device)[:, :, 1]
solutions = torch.tensor(solutions, dtype=dtype, device=device)
...
class DeepONet(nn.Module):
    def __init__(self):
        ...
        self.branch_net = MLPNET(input_dim=input_dim_branch, hidden_layers=hidden_layers_mlp, activation=activation, output_dim=input_dim_branch*HD, dtype=dtype).to(device)
...
loss_df.to_csv('DeepONet_training_testing_losses_seed42.csv', index=False)
print('Training and testing losses have been saved to DeepONet_training_testing_losses.csv')
...
filename_pattern = 'transient_Poisson_results_mlp_file_seed42_part_{}.h5'
